...
namespace HTTP_CC
{
	...
	struct TLSDescriptor {
		...
		TLSDescriptor() {
			descriptor = TlsAlloc();
			if (descriptor == -1) throw std::exception("TlsAlloc() failed to create descriptor");
		};
	...
	public:
		CThread(IRunnable *RunTask, int nID, int nPriority = THREAD_PRIORITY_NORMAL): m_qMutex(), m_nThreadID(nID)
		{
			this->m_bIsInterrupted = false;
			this->m_bIsRunning = false;
			this->m_nThreadPriority = nPriority;
			...
			else throw std::exception("Self referencing not allowed.");
		};
		...
		static CThread& currentThread() 
		{
			...
			if (thr == NULL) 
			{
				throw std::exception("Call is not within a CThread context.");
			}
			return *thr;
		};
			...
			if (this->m_qMutex.TryLock()) 
			{
				if (!this->m_bIsRunning) 
				{
					...
					if (hThread == NULL) 
					{
						this->m_bIsRunning = false;
						this->m_qMutex.Unlock();
						throw std::exception("Failed to call CreateThread(). Thread not started.");
					}
	...
	///////////////////////////////////////////////////////////////////////////////////////////////
	//---------------------------- CLASS ----------------------------------------------------------
	// Helper class to submit tasks to the CThreadPool
	class CPriorityTask {
	private:
		int			m_nPriority;
		IRunnable	*m_pRunObj;
	...
	public:
		...
		bool operator< (const CPriorityTask& rhs) const
		{
			return this->m_nPriority < rhs.m_nPriority;
		}
	...
	*/
	...
	// Every CThreadTask will execute same CThreadPool::run() method.
	...
	public:
		...
		void startAll() {
			for (int i = 0; i < (int)m_arrThreadTasks.size(); i++) {
				m_arrThreadTasks[i]->start();
			}
		};

		// Constructor creates the thread pool and sets capacity for the task queue.
		CThreadPool(unsigned int nThreadsCount, unsigned int nQueueCapacity = 16): 
			m_qMutex(), m_PQueue() 
		{
			int i;
			CThread *thTask = NULL;

			if (nThreadsCount <= 0) throw std::exception("Invalid number of threads supplied.");
			if (nQueueCapacity <= 0) throw std::exception("Invalid capacity supplied.");

			// Set initial capacity of the tasks Queue.
			m_PQueue.reserve(nQueueCapacity);

			// Initialize thread pool.
			for (i = 0; i < (int)nThreadsCount; i++) 
			{
				thTask = new CThread(this, i);
		...
		{
			if (this == pRunObj) throw std::exception("Self referencing not allowed.");
